export function registerAuthRoutes(app, deps) {
  ...
  function buildAuthResponse(session, extra = {}) {
    const accessContext =
      typeof getAccessContextFromSession === "function"
        ? getAccessContextFromSession(session)
        : null;
    const user = accessContext?.user ?? getAuthUserByUsernameFn?.(session?.username);
    const isSuperAdmin = accessContext?.isSuperAdmin ?? (user?.isSuperAdmin === true);
    const accountType =
      accessContext?.accountType ??
      (String(user?.accountType ?? "user")
        ...
        : "user");
    const permissions =
      accessContext?.permissions ?? getRolePermissions(session?.role, { isSuperAdmin });
    const activeProjectCode =
      accessContext?.activeProjectCode ?? getProjectCodeFromSession(session);
    const requiresProjectSelection =
      accessContext?.requiresProjectSelection ?? isProjectSelectionRequired(session);
    const allowedProjectCodes =
      accessContext?.allowedProjects ?? getAllowedProjectsFromSession(session);
    const allowedProjects = getProjectSummaries(allowedProjectCodes);
    const projectAccess = Array.isArray(accessContext?.projectAccess)
      ? accessContext.projectAccess.map((entry) => ({
          projectCode: entry.projectCode,
          projectName: entry.projectCode ? getProjectName(entry.projectCode) : "",
          projectBrandName: entry.projectCode ? getProjectBrandName(entry.projectCode) : "",
          role: entry.role,
    ...
    return {
      authenticated: true,
      username: String(accessContext?.username ?? session?.username ?? ""),
