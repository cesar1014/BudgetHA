export function registerAdminRoutes(app, deps) {
  ...
  app.get("/api/admin/departments", requireSuperAdmin, async (_req, res) => {
    ...
    if (!result.available) {
      res.status(500).json({
        error: `Falha ao carregar departamentos. ${CONSOLIDATED_SCHEMA_GUIDE}`,
      });
      return;
    }
  ...
  app.post("/api/admin/departments/upsert", requireSuperAdmin, async (req, res) => {
    ...
    if (!result.available) {
      res.status(500).json({
        error: `Falha ao gravar departamento. ${CONSOLIDATED_SCHEMA_GUIDE}`,
      });
      return;
    }
  ...
  app.delete("/api/admin/departments/:id", requireSuperAdmin, async (req, res) => {
    ...
    if (!result.available) {
      res.status(500).json({
        error: `Falha ao remover departamento. ${CONSOLIDATED_SCHEMA_GUIDE}`,
      });
      return;
    }
  ...
  app.post("/api/admin/projects/upsert", requireSuperAdmin, async (req, res) => {
    ...
    if (departmentId) {
      ...
      if (!byId.available) {
        res.status(500).json({
          error: `Falha ao gravar departmentId. ${CONSOLIDATED_SCHEMA_GUIDE}`,
        });
        return;
      }
      if (!byId.row) {
    ...
    while (true) {
      ...
      if (missingColumn(error, "status") && includeStatus) {
        if (status === "concluido") {
          res.status(500).json({
            error: `Falha ao gravar status 'concluido'. ${CONSOLIDATED_SCHEMA_GUIDE}`,
          });
          return;
        }
        includeStatus = false;
    ...
    registerProjectCode?.(codeRaw);
    invalidateActiveProjectsCache();
    let bootstrap = null;
    try {
      if (typeof bootstrapProjectRuntime === "function") {
        bootstrap = await bootstrapProjectRuntime(codeRaw);
      }
    } catch (error) {
      res.status(500).json({
        error: `Projeto salvo, mas falha ao preparar estrutura inicial: ${error?.message || error}`,
      });
      return;
    }
